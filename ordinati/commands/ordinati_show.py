...
@click.option('-s',
              ...
              type=click.Choice(['name',
                                 'id',
                                 'url']),
              help='Output to be sorted by this field (id by default).')
@click.option('-c', '--count', default=10, type=click.INT,
              help='Number of bookmarks to be listed (10 by default).')
@click.option(
    '-t',
    '--tag',
    type=click.STRING,
    help='List bookmarks containing the specified tag.')
...
def show(count, sort_by, tag):
    ...
    if count > len(objects):
        click.echo(
            'Warning: {0} exceeds the number of bookmarks.'.format(count))
